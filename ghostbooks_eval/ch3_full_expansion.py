...
with open('ghost_outlines/ch3_synthesis.json', 'rb') as f:
    ch3_scaffold = json.loads(f.read().decode('utf-8-sig'))
...
log_msg('=== CHAPTER 3 FULL PROSE EXPANSION ===')

sections = ch3_scaffold['detailed_outline'].get('sections', [])
expanded_sections = []
...
for sec in sections:
    ...
    system_prompt = 'You are a compelling science writer. Write with clarity, vivid examples, and engagement. Target: 800-1200 words per section. Include modern social media examples, keep narrative strong.'
...
2. Use 2-3 vivid, concrete examples (prefer modern social media/TikTok/LinkedIn context)
3. Explain small-world concepts in accessible language
...
intro_user = f'''Write the introduction to Chapter 3: "Small Worlds & Six Degrees"

Opening Hook (reference):
{ch3_scaffold.get('opening_hook', '')[:400]}...

Create a 300-400 word introduction that:
1. Opens with a modern small-world example (TikTok, LinkedIn, pandemic contact trace)
2. Poses the central question: Why does the world feel so small?
3. Promises readers will understand the mechanism
...
conclusion_system = 'You are writing a chapter conclusion. Synthesize and preview network failures.'
conclusion_user = '''Write the conclusion to Chapter 3: "Small Worlds & Six Degrees"

Key concepts to synthesize:
- Small-world networks create surprising shortcuts
- Clustering + short path lengths = efficiency AND vulnerability
- Algorithms exploit these properties (recommendations)
- This efficiency enables rapid spread (good and bad)

Transition preview: Next chapter explores what happens when networks FAIL or are attacked.
...
with open('ghost_outlines/ch3_FULL_PROSE.json', 'w', encoding='utf-8') as f:
    json.dump(ch3_full, f, indent=2)

log_msg(f'\n=== CHAPTER 3 EXPANSION COMPLETE ===')
log_msg(f'Est. word count: {ch3_full["word_count_estimate"]:,} words')
log_msg(f'Sections: {len(expanded_sections)} complete')
log_msg(f'Saved to: ch3_FULL_PROSE.json')
log_msg(f'\nTotal manuscript so far:')
log_msg(f'  Ch 1: ~5,400 words')
log_msg(f'  Ch 2: ~5,300 words')
log_msg(f'  Ch 3: ~{ch3_full["word_count_estimate"]:,} words')
log_msg(f'  TOTAL: ~{5400 + 5300 + ch3_full["word_count_estimate"]:,} words')
