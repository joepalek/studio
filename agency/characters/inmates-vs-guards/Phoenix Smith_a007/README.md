# Phoenix Smith

**ID:** char-spec-agent-007
**Universe:** inmates-vs-guards
**Archetype:** lifer
**Status:** ungraded
**Created:** 2026-03-30T13:14:48.193052

## Quick Reference

- **Profile:** phoenix_smith_a007-profile.json
- **Backstory:** phoenix_smith_a007-backstory.md
- **Media Log:** logs/phoenix_smith_a007-media-log.json
- **Interaction Log:** logs/phoenix_smith_a007-interaction-log.json
- **Appearances:** logs/phoenix_smith_a007-appearances.json

## Folders

- **images/** -- All character images (portraits, action shots, etc.)
- **video/** -- All video clips featuring character
- **audio/** -- Voice recordings, dialogue, song clips
- **scripts/** -- Scripts for projects this character appears in
- **logs/** -- All tracking logs (media, interactions, appearances)

## Status

- Visual specs: draft
- Locked visual: No

---

*Managed by: Agency Character Creator*
